  ...
  return (
    ...
    >
      <div className="relative">
        <div className="absolute -inset-2 bg-gradient-to-r from-indigo-500/20 via-violet-500/20 to-blue-500/20 rounded-3xl blur-2xl opacity-50" />

        <div className="relative p-6 md:p-10 rounded-2xl bg-gradient-to-br from-indigo-900/30 via-[#0a0a0f] to-violet-900/20 border border-indigo-500/30">
          {/* Grid Background */}
          <div
          ...
          <div className="relative z-10">
                ...
                return (
                  <motion.button
                    ...
                    className={`relative flex items-center gap-2 px-5 py-3 rounded-xl font-medium transition-all ${
                      isActive
                        ? "text-white"
                        : "text-gray-400 hover:text-white bg-gray-900/50 border border-gray-700 hover:border-indigo-500/50"
                    }`}
                ...
                >
                  {[
                    ...
                    return (
                      <motion.div
                        key={stat.label}
                        className="relative p-6 rounded-xl bg-gray-900/50 border border-gray-700 overflow-hidden group hover:border-indigo-500/50 transition-colors"
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
              ...
              {activeTab === "roadmap" && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                  {data.roadmap.phases.map((phase, index) => (
                    ...
                    >
                      {/* Phase Number */}
                      <div className="absolute top-4 right-4 w-10 h-10 rounded-full bg-indigo-500/20 border border-indigo-500/30 flex items-center justify-center">
                        <span className="text-sm font-bold text-indigo-400">{index + 1}</span>
                      </div>

                      {/* Connection Line */}
                      {index < data.roadmap!.phases.length - 1 && (
                        <div className="absolute left-8 top-full w-0.5 h-4 bg-gradient-to-b from-indigo-500 to-transparent" />
                      )}

                      <div className="flex items-start gap-4">
                        <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-indigo-500 to-violet-500 p-[2px] shrink-0">
                          <div className="w-full h-full rounded-xl bg-gray-900 flex items-center justify-center">
                ...
                >
                  {data.roadmap.projects.map((project, index) => {
                    const difficultyColors: Record<string, string> = {
                      ...
                      Advanced: "from-red-500 to-violet-500",
                    }

                    return (
                      <motion.div
                        key={project.title}
                        className="relative p-6 rounded-xl bg-gray-900/50 border border-gray-700 overflow-hidden group hover:border-indigo-500/50 transition-all"
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ delay: index * 0.15 }}
                        whileHover={{ scale: 1.02, y: -4 }}
                      >
                        <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-indigo-500 via-violet-500 to-blue-500" />
                        ...
                        {/* Hover Glow */}
                        <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/0 to-violet-500/0 group-hover:from-indigo-500/5 group-hover:to-violet-500/5 transition-all pointer-events-none" />
                      </motion.div>
                    )
                  })}
                </motion.div>
            ...
            <div className="flex justify-center gap-4 mt-10">
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
              >
                <div className="absolute inset-0 bg-gradient-to-r from-indigo-600 via-violet-600 to-indigo-600 bg-[length:200%_200%] animate-[gradient-shift_8s_ease_infinite]" />
